...
const Page = ({ params }: { params: { id: string } }) => {
  ...
  const oneArticleFetchStatus = useAppSelector(
  ...
  return (
    <form className={styles.container} onSubmit={handleSubmit(onSubmit)}>
      <div className={styles.header}>
        <button type={'submit'} className={global.primaryBtn}>
          Зберегти
        </button>
        <button
          type={'button'}
          className={`${global.primaryBtn} ${global.greyButton}`}
        ...
        <button
          ...
          className={`${global.primaryBtn} ${global.redButton}`}
          onClick={() => {
            if (currentArticle) {
              dispatch(deleteArticle(currentArticle.id)).then(() =>
        ...
        <div className={local.left}>
              ...
              {...register('title', {
                ...
                onChange: (e) => {
                  setValue(
                    `slug`,
                    slugify(
                      e.target.value.replace(/[^a-zA-Zа-яА-Я0-9\s]/g, ''),
                      {
                        lower: true,
                        remove: /[*+~.()''!:@<>$₽]/g,
                      }
          ...
          <div className={global.inputField}>
            ...
            <input
              type="text"
              {...register('author', {
                required: !watch('isNews'),
                minLength: 1,
                disabled: watch('isNews'),
              })}
              className={global.input}
                  ...
                  render={({ field }) => (
                    <LocalizationProvider dateAdapter={AdapterMoment}>
                      <DateTimePicker
                        {...field}
                        minDateTime={moment()}
                        value={moment(watch('eventTime')).utc(true)}
                        ampm={false}
                        label="Вкажіть час"
                      />
        ...
        <div className={local.right}>
          ...
          <div className={`mainImage ${local.mainImage}`}>
            {(!!watch('base64Image') || currentArticle?.imagePath) && (
              <Image
                src={
                  (watch('base64Image') as string) ?? currentArticle?.imagePath
                }
                alt={'image'}
      ...
      <div className={local.sections}>
        ...
        <div className={local.sectionsRight}>
          <button
            className={`${local.prependBtn} ${global.primaryBtn}`}
            onClick={() =>
              append({
                title: '',
                text: '',
