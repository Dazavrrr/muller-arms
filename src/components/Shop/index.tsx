...
const Shop = () => {
  ...
  const categoriesFetchStatus = useAppSelector(state => state.Shop.categoriesFetchStatus)
  const filtersUpdated = useAppSelector(state => state.Shop.filtersUpdated)

  useEffect(() => {
    if (!filtersUpdated){
      dispatch(fetchAllItems({ size: 20, page, sortValue, searchValue, categories: selectedCategories, prices }))
    }
  }, [page, filtersUpdated])
      ...
      <div className={styles.wrapper}>
        ...
        <div className={styles.content}>
          <div className={styles.filters__wrapper}>
            ...
            {filtersUpdated && <button className={`${global.primaryBtn} ${styles.updateBtn}`}
            onClick={() => dispatch(resetFiltersUpd())}>Застосувати</button>}
          </div>
